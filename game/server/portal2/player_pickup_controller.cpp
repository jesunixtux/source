...
#define player_walkspeed hl2_walkspeed
...
bool CWeaponPhysCannon::AttachObject(CBaseEntity *pObject, const Vector &vPosition)
{
	...
	if (pOwner)
	{
		...
		float	maxSpeed = player_walkspeed.GetFloat() * loadWeight;

		//Msg( "Load perc: %f -- Movement speed: %f/%f\n", loadWeight, maxSpeed, player_walkspeed.GetFloat() );
...
void CWeaponPhysCannon::DetachObject(bool playSound, bool wasLaunched)
{
	...
	if (pOwner != NULL)
	{
		pOwner->SetMaxSpeed(player_walkspeed.GetFloat());
